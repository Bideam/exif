	...
	function() {
			...
			$('#getfile').change(function(e){
				EXIF.getData(e.target.files[0],function(){
					...
					$('#showexif').html(EXIF.pretty(this));
...
function showtable(tags){
	if (!tags) {
		var tr=jQuery('<tr>该图片没有EXIF信息</tr>');
		return $('#data-explan').append(tr);
	}
	
	$.each(tags,function(keys){
		for (var i = 0; i < nameasd.length; i++) {
			if(keys == nameasd[i]){
				transexif[value[i]]=tags[keys];
			}
		}
	})
	$.each(transexif,function(keys){
		var tr=jQuery('<tr><td>'+keys+'</td><td>'+transexif[keys]+'</td></tr>');
		$('#data-explan').append(tr);
	})
}

function showimg(file){
	$('#showname').text('上传图片名称：'+file.name);
	$('#showsize').text('上传图片大小：'+(file.size/1024).toFixed(0)+' kb');
	...
	reader.onload=function(e){
		...
		img.id="imgshow"
		img.onload=function(){
			var orwidth=img.width,
				orheight=img.height;
			if (orwidth>orheight) {
				$(this).attr('width','387');
				var scale=orwidth/$(this).attr('width');
				var newheight=orheight/scale;
				$(this).attr('height',newheight);				
			}else{
				$(this).attr('height','387');
				var scale=orheight/$(this).attr('height');
				var newwidth=orwidth/scale;
